...
function Meter () {
    ...
    this.storeSecondInHour = function(power) {
        ...
        logger.info("this is storesecondinhour: ", JSON.stringify(power));
        logger.info("pcount: ", power.pulseCount, JSON.stringify(power.pulsetimes));
        
        Q.when(this.db.rpush("hour", JSON.stringify(power))).then(function () {
            logger.info("store second yay promises work!");
    ...
    this.addTotalDelta = function (power) {
        ...
        logger.info("Change since last: ", value);
