...
function Meter() {
    ...
    this.handleTimer = function () {
        ...
        if (now.getSeconds() === 0) {
            ...
            if (now.getMinutes() === 0) {
                ...
                this.emit("anHour");
                logger.info("emitting testing aWeek");
                this.emit("aWeek");
                if (now.getHours() === 0) {
    ...
    this.storeSecondInHour = function (power) {
        var now              = new Date(),
            ...
            calculatedPulses = Math.round(pulseSum / medianPulse);

        power.pulseCount = parseInt(power.pulseCount, 10);
        power.timestamp  = (now.getTime() - now.getMilliseconds());

        this.pulseCounters.raw      += power.pulseCount;
        this.pulseCounters.median   += (pulseSum / medianPulse);
        ...
        logger.info(
            "calculated pulsecount: ",
            calculatedPulses,
            "",
            (pulseSum / medianPulse)
        );
        logger.info(
            "   average pulsecount: ",
        ...
        if (on.max_deviation > 0.5) {
            ...
            logger.warn(
                "  deviated pulsecount: ",
                power.pulseCount,
                " on max deviation above threshold: ",
                on.max_deviation
            );
        }
        if (on.min_deviation > 0.9) {
            ...
            logger.warn(
                "  deviated pulsecount: ",
                power.pulseCount,
                " on min deviation above threshold: ",
                on.min_deviation
            );
        }
        if (off.max_deviation > 0.5) {
            ...
            logger.warn(
                "  deviated pulsecount: ",
                power.pulseCount,
                " off max deviation above threshold: ",
                off.max_deviation
            );
        }
        if (off.min_deviation > 0.5) {
            ...
            logger.warn(
                "  deviated pulsecount: ",
                power.pulseCount,
                " off min deviation above threshold: ",
                off.min_deviation
            );
        }

        this.pulseCounters.adjusted += power.pulseCount;
        ...
        logger.info(
            "               median: %s %s",
            Math.round(this.pulseCounters.median),
