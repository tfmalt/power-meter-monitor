...
function Meter() {
    ...
    /**
     * This function is run by the monitoring job to actually start 
    ...
    this.startMonitor = function (config) {
        ...
        this.port.open(function () {
            logger.info("Got open from serial port");
            logger.info("Successfully connected to serial port /dev/ttyACM0");
            self.port.on("data", self.handleData);
        });
    ...
    this.handleData = function (data) {
        ...
        logger.info("handleData: ", data, " has Begun:", self.hasBegun);
        if (self.hasBegun === true) {
            logger.info("handleData: got hasBegun:", self.hasBegun);
            ...
            logger.warn("why does I get here");
        }

        self.hasBegun = self.isBeginning(data);
        if (self.hasBegun) {
            logger.info("emitting started");
            self.emit("started");
        }
    };
    ...
    this.isValidData = function (data) {
        ...
        logger.info("isValidData will return true");
        return true;
    };

    this.isBeginning = function (input) {
        logger.info("isBeginning: has begun:", self.hasBegun);
    ...
    this.storeSecondInHour = function (power) {
        ...
        averagePulses += this.pulseCounteres.avgRemainder;

        // Fetches this rounds count and remainder
        var avgCount     = Math.floor(averagePulses);
        var avgRemainder = averagePulses - avgCount;
        ...
        // Logs some stuff.
        logger.info("  median pulsecount:", medCount, medRemainder, medianPulses);
        logger.info(" average pulsecount:", avgCount, avgRemainder, averagePulses);

        if (on.max_deviation > 1.0) {
            adjustedCount += 1;
            logger.warn(
                "deviated pulsecount:",
                adjustedCount,
        ...
        logger.info(
            ...
            this.pulseCounters.avgRemainder
        );
        logger.info(
            "     total adjusted: %s",
