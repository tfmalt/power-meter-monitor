...
function Meter() {
    ...
    // to make sure the application scales properly when deployed in the cloud
    // or on a system with very limited resources like a rasberry pi.
    this.limits = {
    ...
    this.startMonitor = function (config) {
        ...
        this.port = this.getSerialPort();
        this.db   = this.getRedisClient(config);
        this.setupEventHandlers();
    };

    /**
     * Returns the serioal port object for later use
     */
    ...
    // The Meter emits events at given intervals. This is just to have a known
    // set of constants to deal with when emitting events.
    //this.intervals  = {
    //    "tenSeconds":  10 * 1000,
    //    "aMinute":     60 * 1000,
    ...
    /**
     * isValidData returns true if the data passes a very simple validation
     * check
     *
     * @return Boolean
     */
     ...
     * power cycles into indivudual on and off arrays and return that array
     *
     * @return Array
     */
