...
function Meter () {
    ...
    this.storeSecondInHour = function(power) {
        ...
        logger.info("  original pulsecount: ", power.pulseCount);

        var pulses = {on: [], off: []};
        power.pulsetimes.forEach( function (item) {
            if (item == 0) return;
        ...
        var on = {
            max:     u.max(pulses.on),
            min:     u.min(pulses.on),
            average: this.average(pulses.on),
            median:  this.median(pulses.on),
        };
        on.max_deviation = (on.max-on.median)/on.median;
        on.min_deviation = (on.median-on.min)/on.median;

        var off = {
            max:     u.max(pulses.off),
            min:     u.min(pulses.off),
            average: this.average(pulses.off),
            median:  this.median(pulses.off)
        };

        off.max_deviation = (off.max-off.median)/off.median;
        off.min_deviation = (off.median-off.min)/off.median;

        var sum = pulses.on.concat(pulses.off).reduce(function (prev, curr) {
            return prev+curr;
        });
        ...
        // logger.info("pulses on : ", pulses.on);
        // logger.info("on stats: ", on);
        // logger.info("pulses off: ", pulses.off);
        // logger.info("off stats: ", off);
        logger.info(
